import { _decorator, Component, Prefab, Label, Sprite, Node, systemEvent, SystemEvent, UIOpacity, instantiate, color, macro, Vec3, tween, easing, v2, Vec2, sys, input, Input, EventKeyboard, KeyCode, Graphics, UITransform } from 'cc';
...
export class Grid extends Component {
    ...
    @property(Label)
    bestScore: Label | null = null
    @property(UIOpacity)
    gameover: UIOpacity | null = null
    ...
    init() {
        ...
        this.gameover.opacity = 0 // 隐藏gameover弹窗
        ...
        this.walkCells((i,) => {
            ...
            for (let j = 0; j < this.col; ++j) {
                ...
                cell.getComponent(Sprite).color = color(238, 228, 218, 0.35)
    ...
    createCell(i: number, j: number) {
        ...
        // 进场动画
        cell.setScale(new Vec3(0.5, 0.5, 0.5))
        tween(cell).to(0.5, { scale: new Vec3(1, 1, 1) }, { easing: easing.cubicOut }).start()
    }
    //    // 找到一个随机的空闲位置并生成cell
    createRandomCell() {
        let freeCells = []
    ...
    setCellPosition(cell, i, j, needAction = false, cb = () => { }) {
        const width = 100
        const height = 100
        let target = v2((50 + j * width), -50 - i * height)
        console.log(target)
        if (needAction) {
            tween(cell)
                .set({ position: new Vec2(0.5, 0.5) })
    ...
    gameDie() {
        this.gameover.opacity = 1
        // 保存最高得分
        let bestScore = this.getBestScore()
        let currentScore = this.score.string
        if (currentScore > bestScore) {
            sys.localStorage.setItem(BEST_SCORE_KEY, currentScore);
        }
    }
    replayGame() {
        this.init()
    ...
    drawMap() {
        ...
        const { row, col } = this
        let graphics: Graphics = node.addComponent(Graphics)
        graphics.lineWidth = 1
        graphics.strokeColor = color(255, 0, 0, 255)
        for (let i = 0; i <= row; ++i) {
            let y = i * height / row
            graphics.moveTo(0, -y)
